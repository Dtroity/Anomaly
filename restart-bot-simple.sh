...
# Простой скрипт для перезапуска бота с новыми настройками
...
# 2. Просто перезапустить контейнер (это перезагрузит переменные окружения)
echo "🔄 Перезапуск контейнера бота..."
docker restart anomaly-bot 2>/dev/null || {
    echo "  ⚠️  docker restart не сработал, используем docker-compose..."
...
MARZBAN_URL_IN_CONTAINER=$(docker-compose exec -T bot env 2>/dev/null | grep "MARZBAN_API_URL" | cut -d'=' -f2 || echo "не найдено")
